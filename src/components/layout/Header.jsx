  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-50" dir="rtl">
      <nav className="container-custom flex items-center justify-between py-4" aria-label="Global">
        {/* Mobile menu button - Now on the left for RTL */}
        ...
        {/* Desktop navigation - Now on the right for RTL */}
        <div className="hidden lg:flex lg:gap-x-8">
          {navigation.map((item) => (
            ...
            >
              {item.name}
              <span className="absolute bottom-0 right-0 w-0 h-0.5 bg-gold-600 transition-all duration-300 group-hover:w-full"></span>
            </Link>
          ))}
        </div>

        {/* Logo - Now on the left for RTL */}
                ...
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <Link
                      key={item.name}
                      to={item.href}
                      className="arabic-text -mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gold-50 hover:text-gold-600 transition-colors text-right"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {item.name}
                    </Link>
